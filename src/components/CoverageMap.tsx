...
export function CoverageMap({ representatives }: CoverageMapProps) {
  ...
  useEffect(() => {
    ...
    // Desenha o Brasil todo em vermelho (zonas não atendidas)
    const brazilBounds: L.LatLngBoundsExpression = [
      [-33.7, -73.9],  // sudoeste (SW)
      [5.3, -28.8],    // nordeste (NE)
    ];
    ...
    for (const rep of repsWithCoords) {
      ...
      // Atendimento Possível TLV 150-300km  
      L.circle(pos, {
        radius: 300000,
        color: ZONE_COLORS.possivel_tlv,
        fillColor: ZONE_COLORS.possivel_tlv,
        fillOpacity: 0.1,
        weight: 1,
      }).addTo(map);
